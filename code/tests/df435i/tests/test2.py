...
# Configure streams
config = rs.config()
config.enable_stream(rs.stream.depth, 640, 480, rs.format.z16, 30) # without this: invalid depth
config.enable_stream(rs.stream.color, 640, 480, rs.format.bgr8, 30) # without this: invalid color

# Start streaming
pipeline.start(config)

pipeline.wait_for_frames() # without this first call doesn't work: don't know why
