import sys
sys.path.append("PyKinect2") # workspace folder = kinect1656
"""
...
def depth_2_color_space(kinect, depth_space_point, depth_frame_data, show=False, return_aligned_image=False):
    ...
    if show:
        ...
        # cv2.waitKey(3000) # slows down program
...
if __name__ == '__main__':
    ...
    while True:
        if kinect.has_new_depth_frame():
            ...
            cv2.circle(colorImage, (400,500), radius=30, color=(255, 0, 0), thickness=-1)
            cv2.circle(colorImage, (400,500), radius=10, color=(0, 0, 255), thickness=-1)
            # colorImage = cv2.flip(colorImage, 1)

            end = time.time()
            fps = round(1/(end-start),2)

            cv2.putText(colorImage, f'FPS: {fps}', (20,50), cv2.FONT_HERSHEY_COMPLEX, 2, (0,0,255))
            ...
            cv2.putText(depth_img, f'FPS: {fps}', (20,50), cv2.FONT_HERSHEY_COMPLEX, 1.3, (0,0,255))
            cv2.imshow('Test Depth View', depth_img)
            start = time.time()
            print(color_point_2_depth_point(kinect, _DepthSpacePoint, kinect._depth_frame_data, [400,500]))
            depth_x, depth_y = color_point_2_depth_point(kinect, _DepthSpacePoint, kinect._depth_frame_data, [400,500])
            print(f'depth(100,100):: {depth_space_2_world_depth(depth_frame, depth_x, depth_y)}')
            ...
            # depth_2_color_space(kinect, _DepthSpacePoint, kinect._depth_frame_data, show=False)
