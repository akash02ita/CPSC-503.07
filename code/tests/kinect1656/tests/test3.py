"""
    ...
    applying opencv filtering for depth
"""
import sys
...
def post_process_depth_image(depth_image):
    # https://docs.opencv.org/4.x/d4/d13/tutorial_py_filtering.html
    depth_image = cv2.bilateralFilter(depth_image, 9, 75, 75) # using same values from link. Also bilateralFilter is quite slow and hardware intensive. So parameters should be not too high.
...
while True:
    ...
    if kinect.has_new_depth_frame():
        ...
        cv2.imshow("depth-before", depth_image)

        depth_image = post_process_depth_image(depth_image)

        # cv2.UMat(depth_image)
        cv2.imshow("depth-after", depth_image)
